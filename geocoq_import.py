...
class LemmaPrinter:
    ...
    def assert_by_conclude_def_Lt_destruct(self, a: LtacAssertBy) -> None:
        e = a.prop
        assert isinstance(e, PropConjunction)
        assert isinstance(e.left, PropSimple)
        assert isinstance(e.right, PropSimple)
        BetS_C_X_D = e.left
        Cong_C_X_A_B = e.right
        C, X, D = BetS_C_X_D.points
        C, X, A, B = Cong_C_X_A_B.points
        self.w.write(f"destruct Lt_{A}_{B}_{C}_{D} as ({X} & {a.prop.to_var()}).\n")

    def assert_by_conclude_def_Lt(self, a: LtacAssertBy) -> None:
        if isinstance(a.prop, PropConjunction):
            self.assert_by_conclude_def_Lt_destruct(a)
            return
...
bad_path = "lemma_lessthanadditive"
