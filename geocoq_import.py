...
lemmas_should_be_forward_using = [
...
class LemmaPrinter:
    ...
    def assert_by_cn_equalitysub(self, a: LtacAssertBy) -> None:
        ...
        match len(h.points):
            case 2:
                ...
                # fmt: off
                self.process_indent()
                self.w.write(f"assert ({h.head} {A} {B}) as {h.head}_{A}_{B} by (rewrite eq_{A}_X; exact {h.head}_X_{B}).\n")
                self.process_indent()
                self.w.write(f"assert ({h.head} {A} {B}) as {h.head}_{A}_{B} by (rewrite eq_{B}_X; exact {h.head}_{A}_X).\n")
                self.process_indent()
                self.w.write(f"assert ({h.head} {A} {B}) as {h.head}_{A}_{B} by (rewrite <- eq_X_{A}; exact {h.head}_X_{B}).\n")
                self.process_indent()
                self.w.write(f"assert ({h.head} {A} {B}) as {h.head}_{A}_{B} by (rewrite <- eq_X_{B}; exact {h.head}_{A}_X).\n")
                # fmt: on
                self.w.write("\n")
            case 3:
                A, B, C = h.points
            ...
            case 4:
                ...
                self.w.write("\n")
            case _:
                raise NotImplementedError("equalitysub with not 2,3,4 points is not yet done")
    ...
    def assert_by(self, a: LtacAssertBy) -> None:
        if a.by.t == "conclude_def":
            ...
            return
        if a.by.t == "forward_using" or a.by.n in lemmas_should_be_forward_using:
