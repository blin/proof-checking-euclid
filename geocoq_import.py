...
def prove_simple(p: PropSimple, by: Lemma, context: list[Prop]) -> list[Prop]:
    ...
    l_i = f"pose proof ({by.name} {pp} {gg}) as {p.to_var()}"
    print(l_i)
...
req_names = extract_requirements_top(top) - {"Triangle", "isosceles"}
...
def prolog_prop(p: Prop, idp: str, p_to_i: dict[str, int]) -> str:
    r = io.StringIO()
    conclusion = idp[0] == "C"
...
for req_name in sorted(req_names):
    ...
    for i, p in enumerate(req.given):
        r.write(prolog_prop(p, f"G{i+1}", p_to_i))
    r.write(prolog_prop(req.conclusion, f"C1", p_to_i))
    rr.write(r.getvalue())
    rr.write("\n")
rr.close()
