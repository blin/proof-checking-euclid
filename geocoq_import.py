...
class LemmaPrinter:
    ...
    def assert_by_conclude_def(self, a: LtacAssertBy) -> None:
        ...
        match a.by.n:
            ...
            case "InCirc":
                P, J = a.prop.points
                U, V, W, X, Y = "U", "V", "W", "X", "Y"
                self.w.write(f"pose proof (lemma_s_incirc_centre _ _ _ _ CI_{J}_{P}_{V}_{W}) as {a.prop.to_var()}.\n")
                self.process_indent()
                self.w.write(
            ...
            case "OnCirc":
                self.assert_by(LtacAssertBy(a.prop, by=LtacConclude(t="conclude", n="lemma_s_oncirc_radius")))
            case "OutCirc":
                self.assert_by(LtacAssertBy(a.prop, by=LtacConclude(t="conclude", n="lemma_s_outcirc_beyond_perimeter")))
            case "OnRay":
                self.assert_by(LtacAssertBy(a.prop, by=LtacConclude(t="conclude", n="lemma_s_onray")))
            case _:
    ...
    def assert_by_forward_using_lemma_NCorder(self, a: LtacAssertBy):
        e = a.prop
        hhs = self.context.get_props()
        hhs = [h for h in hhs if isinstance(h, PropSimple)]
        ...
        for hh in hhs:
            ...
            X, Y, Z = hh.points
            self.w.write(
                f"pose proof (lemma_NCorder _ _ _ nCol_{X}_{Y}_{Z}) as (nCol_{Y}_{X}_{Z} & nCol_{Y}_{Z}_{X} & nCol_{Z}_{X}_{Y} & nCol_{X}_{Z}_{Y} & nCol_{Z}_{Y}_{X}). (* wanted {e.to_var()} *)\n"
            )
            break

    def assert_by_forward_using_lemma_NCdistinct(self, a: LtacAssertBy):
        e = a.prop
        hhs = self.context.get_props()
        hhs = [h for h in hhs if isinstance(h, PropSimple)]

        for hh in hhs:
            if not ((e.points[0] in set(hh.points)) and (e.points[1] in set(hh.points))):
                continue
            X, Y, Z = hh.points
            self.w.write(
                f"pose proof (lemma_NCdistinct _ _ _ nCol_{X}_{Y}_{Z}) as (neq_{X}_{Y} & neq_{Y}_{Z} & neq_{X}_{Z} & neq_{Y}_{X} & neq_{Z}_{Y} & neq_{Z}_{X}). (* wanted {e.to_var()} *)\n"
            )
            break
    ...
    def assert_by_forward_using(self, a: LtacAssertBy) -> None:
        ...
        match a.by.n:
            ...
            case "lemma_NCorder":
                self.assert_by_forward_using_lemma_NCorder(a)
            case "lemma_NCdistinct":
                self.assert_by_forward_using_lemma_NCdistinct(a)
            case _:
                prolog_context(self.context.get_props(), self.prolog_context_path)

                proof_lemma = self.lemmas_by_name[a.by.n]
                assert isinstance(proof_lemma.conclusion, PropConjunction)
                conclusions = collect_conjunction_nodes(proof_lemma.conclusion)

                proof, ci = prove_forward_using(a.prop, proof_lemma, self.prolog_lemmas_path, self.prolog_context_path)
    ...
    def assert_by_lemma_extension(self, a: LtacAssertBy) -> None:
        ...
        self.w.write(f"pose proof (lemma_extension {A} {B} {P} {Q} neq_{A}_{B} neq_{P}_{Q}) as ({X} & {e.to_var()}).\n")
    ...
    def assert_by(self, a: LtacAssertBy) -> None:
        ...
        match a.prop:
            case PropSimple():
                prolog_context(self.context.get_props(), self.prolog_context_path)
                proof = prove_simple(a.prop, proof_lemma, self.prolog_lemmas_path, self.prolog_context_path)
                self.context.add_prop(a.prop)
            ...
            case PropExists():
                match a.prop.p:
                    case PropSimple():
                        prolog_context(self.context.get_props(), self.prolog_context_path)
                        proof = prove_simple(a.prop.p, proof_lemma, self.prolog_lemmas_path, self.prolog_context_path)
    ...
    def prepare_prolog(self) -> None:
        prolog_lemmas(extract_requirements_lemma(self.l), self.lemmas_by_name, self.prolog_lemmas_path)
