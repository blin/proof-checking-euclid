...
Proof.
	...
	destruct Perp_at_M_Q_A_B_Q as (E & Col_M_Q_Q & Col_A_B_Q & Col_A_B_E & RightTriangle_E_Q_M).

	assert (~ eq M Q) as n_eq_M_Q.
	{
		intro eq_M_Q.
		assert (Col A B M) as Col_A_B_M by (rewrite eq_M_Q; exact Col_A_B_Q).
		contradict Col_A_B_M.
		exact n_Col_A_B_M.
	}
	assert (neq_M_Q := n_eq_M_Q).


	pose proof (lemma_inequalitysymmetric _ _ neq_M_Q) as neq_Q_M.
	...
	assert (~ eq M G) as n_eq_M_G.
	{
		intro eq_M_G.
		assert (BetS Q M C) as BetS_Q_M_C by (rewrite eq_M_G; exact BetS_Q_G_C).
		pose proof (lemma_s_col_BetS_A_B_C Q M C BetS_Q_M_C) as Col_Q_M_C.
		pose proof (lemma_collinear_ABC_ABD_BCD _ _ _ _ Col_A_B_Q Col_A_B_C neq_A_B) as Col_B_Q_C.
		pose proof (lemma_collinearorder _ _ _ Col_Q_M_C) as (_ & _ & _ & Col_Q_C_M & _).
		pose proof (lemma_collinearorder _ _ _ Col_B_Q_C) as (_ & Col_Q_C_B & _ & _ & _).
		pose proof (lemma_collinear_ABC_ABD_BCD _ _ _ _ Col_Q_C_M Col_Q_C_B neq_Q_C) as Col_C_M_B.
		pose proof (lemma_collinearorder _ _ _ Col_C_M_B) as (_ & _ & _ & Col_C_B_M & _).
		pose proof (lemma_collinearorder _ _ _ Col_B_A_C) as (_ & _ & Col_C_B_A & _ & _).
		pose proof (lemma_betweennotequal _ _ _ BetS_A_C_B) as (neq_C_B & _ & _).
		pose proof (lemma_collinear_ABC_ABD_BCD _ _ _ _ Col_C_B_M Col_C_B_A neq_C_B) as Col_B_M_A.
		pose proof (lemma_collinearorder _ _ _ Col_B_M_A) as (_ & _ & Col_A_B_M & _ & _).
		contradict Col_A_B_M.
		exact n_Col_A_B_M.
	}
	assert (neq_M_G := n_eq_M_G).
	...
	pose proof (lemma_collinearorder _ _ _ Col_Q_G_C) as (_ & _ & Col_C_Q_G & _ & _).
	...
	assert (~ eq J G) as n_eq_J_G.
	{
		intro eq_J_G.
		pose proof (lemma_s_col_eq_A_C J Q G eq_J_G) as Col_J_Q_G.
		pose proof (lemma_right_triangle_NC _ _ _ RightTriangle_J_Q_G) as nCol_J_Q_G.
		pose proof (lemma_s_ncol_n_col _ _ _ nCol_J_Q_G) as n_Col_J_Q_G.
		contradict Col_J_Q_G.
		exact n_Col_J_Q_G.
	}
	assert (neq_J_G := n_eq_J_G).
	...
	pose proof (lemma_collinearorder _ _ _ Col_C_Q_G) as (Col_Q_C_G & _ & _ & _ & _).
