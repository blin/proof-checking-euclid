...
Proof.
	...
	pose proof (by_prop_Col_order _ _ _ Col_A_B_C) as (_ & _ & _ & Col_A_C_B & _).

	pose proof (by_prop_nCol_distinct _ _ _ nCol_A_C_E) as (neq_A_C & neq_C_E & neq_A_E & neq_C_A & neq_E_C & neq_E_A).
	...
	assert (~ ~ OppositeSide G A C Q) as n_n_OppositeSide_G_AC_Q.
	{
		...
		assert (~ eq B V) as n_eq_B_V.
		{
			...
			assert (~ Col A C G) as n_Col_A_C_G.
			{
				...
				{
					intro neq_B_C.
					pose proof (by_prop_Col_order _ _ _ Col_A_B_C) as (_ & Col_B_C_A & _ & _ & _).
		...
		assert (~ Col Q F B) as n_Col_Q_F_B.
		{
			...
			pose proof (by_prop_Col_order _ _ _ Col_A_C_V) as (_ & _ & _ & _ & Col_V_C_A).
			...
			pose proof (by_prop_Col_order _ _ _ Col_A_C_B) as (Col_C_A_B & _ & _ & _ & _).
			pose proof (by_prop_Col_order _ _ _ Col_A_C_V) as (Col_C_A_V & _ & _ & _ & _).
			...
			pose proof (by_prop_Col_order _ _ _ Col_A_C_V) as (_ & _ & Col_V_A_C & _ & _).
		...
		}
		{
			...
			{
				intro Col_B_G_Q.
				pose proof (by_prop_BetS_notequal _ _ _ BetS_B_F_G) as (_ & neq_B_F & neq_B_G).
				pose proof (by_prop_neq_symmetric _ _ neq_B_G) as neq_G_B.
