...
Proof.
	...
	assert (~ ~ TarksiPar A B C D) as n_n_TarksiPar_A_B_C_D.
	{
		...
		{
			intro neq_C_d.
			pose proof (lemma_parallelcollinear _ _ _ _ _ TarksiPar_A_B_c_d Col_c_d_C neq_C_d) as TarksiPar_A_B_C_d.
			...
			pose proof (lemma_parallelcollinear _ _ _ _ _ TarksiPar_A_B_d_C Col_d_C_D neq_D_C) as TarksiPar_A_B_D_C.
			pose proof (lemma_tarskiparallelflip _ _ _ _ TarksiPar_A_B_D_C) as (_ & TarksiPar_A_B_C_D & _).
			contradict TarksiPar_A_B_C_D.
			exact n_TarksiPar_A_B_C_D.
		}
		assert (eq_C_d := n_neq_C_d).
		apply Classical_Prop.NNPP in eq_C_d.


		assert (TarksiPar A B c C) as TarksiPar_A_B_c_C by (rewrite eq_C_d; exact TarksiPar_A_B_c_d).
		pose proof (lemma_collinearorder _ _ _ Col_C_D_c) as (_ & _ & Col_c_C_D & _ & _).
		pose proof (lemma_parallelcollinear _ _ _ _ _ TarksiPar_A_B_c_C Col_c_C_D neq_D_C) as TarksiPar_A_B_D_C.
		pose proof (lemma_tarskiparallelflip _ _ _ _ TarksiPar_A_B_D_C) as (_ & TarksiPar_A_B_C_D & _).
		contradict TarksiPar_A_B_C_D.
		exact n_TarksiPar_A_B_C_D.
	}
