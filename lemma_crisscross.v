...
Proof.
	...
	pose proof (lemma_paralleldef2B _ _ _ _ Par_B_D_A_C) as TP_B_D_A_C.

	destruct TP_B_D_A_C as (neq_B_D & neq_A_C & n_Meet_B_D_A_C & SS_A_C_B_D).
	...
	(* assert by cases *)
	assert (CR A D B C) as CR_A_D_B_C.
	...
	}
	{
		...
		assert (~ ~ CR A D B C) as n_n_CR_A_D_B_C.
		{
			intro n_CR_A_D_B_C.
		...
		assert (CR_A_D_B_C := n_n_CR_A_D_B_C).
		apply Classical_Prop.NNPP in CR_A_D_B_C.
	...
	}
	{
		...
		assert (~ ~ CR A D B C) as n_n_CR_A_D_B_C.
		{
			...
			assert (CR A B C D) as CR_A_B_C_D.
			unfold CR.
			exists M.
			split.
			exact BetS_A_M_B.
