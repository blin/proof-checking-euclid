...
Proof.
	...
	destruct SS_E_F_A_C as (Q & U & V & Col_A_C_U & Col_A_C_V & BetS_E_U_Q & BetS_F_V_Q & nCol_A_C_E & nCol_A_C_F).

	pose proof (lemma_s_os _ _ _ _ _ BetS_F_V_Q Col_A_C_V nCol_A_C_F) as OS_F_A_C_Q.
	...
	pose proof (lemma_collinearorder _ _ _ Col_B_F_G) as (_ & _ & _ & Col_B_G_F & _).

	assert (~ ~ OS G A C Q) as OS_G_A_C_Q.
	{
		intros n_OS_G_A_C_Q.

		assert (~ eq F G) as neq_F_G.
		{
			intros eq_F_G.

			assert (OS G A C Q) as OS_G_A_C_Q by (rewrite <- eq_F_G; exact OS_F_A_C_Q).

			contradict OS_G_A_C_Q.
			exact n_OS_G_A_C_Q.
		}

		assert (~ eq B V) as neq_B_V.
		{
			intros eq_B_V.

			assert (BetS F B Q) as BetS_F_B_Q by (rewrite eq_B_V; exact BetS_F_V_Q).
			pose proof (lemma_onray_orderofpoints_any _ _ _ OnRay_B_F_G) as BetS_B_G_F_or_eq_F_G_or_BetS_B_F_G.
			assert (BetS G B Q) as BetS_G_B_Q.
			destruct BetS_B_G_F_or_eq_F_G_or_BetS_B_F_G as [BetS_B_G_F|[eq_F_G|BetS_B_F_G]].
			{
				(* case BetS_B_G_F *)
			...
			}
			{
				...
				{
					intros nBetS_G_B_Q.
					assert (~ BetS F B Q) as nBetS_F_B_Q by (rewrite eq_F_G; exact nBetS_G_B_Q).
					contradict BetS_F_B_Q.
					exact nBetS_F_B_Q.
				}
				apply Classical_Prop.NNPP in BetS_G_B_Q.
				exact BetS_G_B_Q.
			}
			{
				(* case BetS_B_F_G *)
			...
			assert (~ Col A C G) as n_Col_A_C_G.
			{
				...
				pose proof (lemma_collinearorder _ _ _ Col_B_G_F) as (Col_G_B_F & _ & _ & _ & _).
			...
			pose proof (lemma_s_os _ _ _ _ _ BetS_G_B_Q Col_A_C_B nCol_A_C_G) as OS_G_A_C_Q.
		...
		{
			(* case BetS_B_G_F *)
			pose proof (lemma_otherside_betweenness_PABC_RQP_QABC _ _ _ _ _ _ OS_F_A_C_Q BetS_B_G_F nCol_Q_F_B Col_A_C_B) as OS_G_A_C_Q.
			contradict OS_G_A_C_Q.
			exact n_OS_G_A_C_Q.
		}
		{
		...
		}
		{
			...
			assert (~ Col B G Q) as n_Col_B_G_Q.
			{
				...
				pose proof (lemma_betweennotequal _ _ _ BetS_B_F_G) as (_ & _ & neq_B_G).
				pose proof (lemma_inequalitysymmetric _ _ neq_B_G) as neq_G_B.
			...
			pose proof (lemma_otherside_betweenness_PABC_RPQ_QABC _ _ _ _ _ _ OS_F_A_C_Q BetS_B_F_G nCol_B_G_Q Col_A_C_B) as OS_G_A_C_Q.
			contradict OS_G_A_C_Q.
			exact n_OS_G_A_C_Q.
		}
	}
	apply Classical_Prop.NNPP in OS_G_A_C_Q.

	destruct OS_G_A_C_Q as (H & BetS_G_H_Q & Col_A_C_H & nCol_A_C_G).
	unfold SS.
	exists Q.
	exists U.
	exists H.
