...
Lemma lemma_tarskiparallelflip :
	forall A B C D,
	TarksiPar A B C D ->
	TarksiPar B A C D /\ TarksiPar A B D C /\ TarksiPar B A D C.
Proof.
	intros A B C D.
	...
	assert (TarksiPar A B D C) as TarksiPar_A_B_D_C.
	unfold TarksiPar.
	split.
	exact neq_A_B.
	split.
	...
	assert (TarksiPar B A C D) as TarksiPar_B_A_C_D.
	unfold TarksiPar.
	split.
	exact neq_B_A.
	split.
	...
	assert (TarksiPar B A D C) as TarksiPar_B_A_D_C.
	unfold TarksiPar.
	split.
	exact neq_B_A.
	split.
	exact neq_D_C.
	split.
	exact n_Meet_B_A_D_C.
	exact SS_D_C_B_A.

	split.
	exact TarksiPar_B_A_C_D.
	split.
	exact TarksiPar_A_B_D_C.
	exact TarksiPar_B_A_D_C.
Qed.
