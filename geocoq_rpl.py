...
class NodeVisitor:
    ...
    def visit_ltac_expr_ltac_expr4(self, node: Node, vc: list[Any]):
        if len(vc) != 1:
            raise ValueError(f"ltac_expr4 with braces or semicolons: {node}")
        return vc[0]
    ...
    def visit_ltac_expr_ltac_expr1(self, node: Node, vc: list[Any]):
        ...
        if tactic_name == "assert":
            ...
            prop = vc[1]
            assert isinstance(prop, get_args(Prop)), f"assert with unknown prop: {node}"
            if len(vc) == 2:
                return LtacAssert(prop=prop)
            by = vc[3]
            assert isinstance(by, LtacConclude), f"assert with unknown by: {node}"
    ...
    def visit_lemma_intro(self, node: Node, vc: list[Any]):
        ...
        assert isinstance(conclusion, get_args(Prop))
