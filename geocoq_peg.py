...
# TODO: next pose proof (axiom_betweennesssymmetry) as BetS_C_E_A.
...
def cn_equalityreflexive(h: Hypothesis, w: TextIO) -> None:
    A, A = h.points
...
def proposition_04(e: Conjunction, w: TextIO) -> None:
...
def lemma_angleorderrespectscongruence(e: Hypothesis, w: TextIO):
    ...
    w.write(
        f"pose proof (lemma_angleorderrespectscongruence {A} {B} {C} X Y Z {a} {b} {c} LtA_{A}_{B}_{C}_X_Y_Z CongA_{a}_{b}_{c}_X_Y_Z) as {e.to_var()}.\n"
    )
...
def lemma_angleorderrespectscongruence_smaller(e: Hypothesis, w: TextIO):
    ...
    w.write(
        f"pose proof (lemma_angleorderrespectscongruence_smaller X Y Z {D} {E} {F} {a} {b} {c} LtA_X_Y_Z_{D}_{E}_{F} CongA_{a}_{b}_{c}_X_Y_Z) as {e.to_var()}.\n"
    )
...
def lemma_equalangleshelper(e: Hypothesis, w: TextIO):
    ...
    w.write(
        f"pose proof (lemma_equalangleshelper {A} {B} {C} X {b} Z {p} {q} CongA_{A}_{B}_{C}_X_{b}_Z OnRay_{b}_X_{p} OnRay_{b}_Z_{q}) as {e.to_var()}.\n"
    )
...
def lemma_equalanglestransitive(e: Hypothesis, w: TextIO):
    ...
    w.write(
        f"pose proof (lemma_equalanglestransitive {A} {B} {C} X Y Z {P} {Q} {R} CongA_{A}_{B}_{C}_X_Y_Z CongA_X_Y_Z_{P}_{Q}_{R}) as {e.to_var()}.\n"
    )
