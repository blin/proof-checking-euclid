...
def proposition_10(e: Conjunction, w: TextIO) -> None:
    ...
    w.write(
        f"pose proof (proposition_10 {A} {B} neq_{A}_{B}) as ({X} & {e.to_var()}).\n"
    )
...
def lemma_inequalitysymmetric(h: Hypothesis, w: TextIO) -> None:
    B, A = h.points
    w.write(
        f"pose proof (lemma_inequalitysymmetric {A} {B} neq_{A}_{B}) as neq_{B}_{A}.\n"
    )
...
def lemma_betweennotequal(e: Hypothesis, w: TextIO):
    A, B = e.points
    w.write(
        f"pose proof (lemma_betweennotequal _ _ _ BetS_X_{A}_{B}) as (neq_{A}_{B} & _ & _).\n"
    )
    w.write(
        f"pose proof (lemma_betweennotequal _ _ _ BetS_{A}_{B}_Z) as (_ & neq_{A}_{B} & _).\n"
    )
    w.write(
        f"pose proof (lemma_betweennotequal _ _ _ BetS_{A}_Y_{B}) as (_ & _ & neq_{A}_{B}).\n"
    )


def lemma_ABCequalsCBA(e: Hypothesis, w: TextIO):
    A, B, C, C, B, A = e.points
    w.write(f"pose proof (lemma_ABCequalsCBA {A} {B} {C}) as {e.to_var()}.\n")


def lemma_angleorderrespectscongruence(e: Hypothesis, w: TextIO):
    A, B, C, a, b, c = e.points
    w.write(
        f"pose proof (lemma_angleorderrespectscongruence {A} {B} {C}) as {e.to_var()}.\n"
    )
...
def lemma_angleorderrespectscongruence_smaller(e: Hypothesis, w: TextIO):
    ...
    w.write(
        f"pose proof (lemma_angleorderrespectscongruence_smaller {a} {b} {c}) as {e.to_var()}.\n"
    )


def lemma_collinear_ABC_ABD_BCD(e: Hypothesis, w: TextIO):
    B, C, D = e.points
    w.write(f"pose proof (lemma_collinear_ABC_ABD_BCD {D} {B} {C}) as {e.to_var()}.\n")


def lemma_collinearorder(e: Hypothesis, w: TextIO):
    # TODO: expansion
    A, B, C = e.points
    w.write(f"pose proof (lemma_collinearorder {A} {B} {C}) as {e.to_var()}.\n")
