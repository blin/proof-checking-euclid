    ...
    def to_str(self, indent: int, stack: list[list[Any]]) -> str:
        ...
        if isinstance(self.e, Inversion):
            if self.e.h.head in ["Col"]:
                w.write("\t" * indent)
                w.write(
                    f"pose proof (lemma_s_n_col_ncol _ _ _ n_{self.e.to_var()}) as n{self.e.to_var()}.\n"
                )
...
def cn_equalitysub(h: Hypothesis, stack: list[list[Any]], w: TextIO) -> None:
    ...
    w.write("\n")
    w.write(
        f"assert ({h.head} {A} {B} {C}) as {h.head}_{A}_{B}_{C} by (rewrite eq_{A}_X; exact {h.head}_X_{B}_{C}).\n"
    )
    w.write(
        f"assert ({h.head} {A} {B} {C}) as {h.head}_{A}_{B}_{C} by (rewrite eq_{B}_X; exact {h.head}_{A}_X_{C}).\n"
    )
    w.write(
        f"assert ({h.head} {A} {B} {C}) as {h.head}_{A}_{B}_{C} by (rewrite eq_{C}_X; exact {h.head}_{A}_{B}_X).\n"
    )
    w.write(
        f"assert ({h.head} {A} {B} {C}) as {h.head}_{A}_{B}_{C} by (rewrite <- eq_X_{A}; exact {h.head}_X_{B}_{C}).\n"
    )
    w.write(
        f"assert ({h.head} {A} {B} {C}) as {h.head}_{A}_{B}_{C} by (rewrite <- eq_X_{B}; exact {h.head}_{A}_X_{C}).\n"
    )
    w.write(
        f"assert ({h.head} {A} {B} {C}) as {h.head}_{A}_{B}_{C} by (rewrite <- eq_X_{C}; exact {h.head}_{A}_{B}_X).\n"
    )
    w.write("\n")
...
def lemma_angledistinct(h: Hypothesis, stack: list[list[Any]], w: TextIO) -> None:
    ...
    w.write("\n")
    w.write(
        f"pose proof (lemma_angledistinct _ _ _ _ _ _ CongA_{A}_{B}_Z_x_y_z) as (neq_{A}_{B} & _ & _ & _ & _ & _).\n"
    )
    w.write(
        f"pose proof (lemma_angledistinct _ _ _ _ _ _ CongA_X_{A}_{B}_x_y_z) as (_ & neq_{A}_{B} & _ & _ & _ & _).\n"
    )
    w.write(
        f"pose proof (lemma_angledistinct _ _ _ _ _ _ CongA_{A}_Y_{B}_x_y_z) as (_ & _ & neq_{A}_{B} & _ & _ & _).\n"
    )
    w.write(
        f"pose proof (lemma_angledistinct _ _ _ _ _ _ CongA_X_Y_Z_{A}_{B}_z) as (_ & _ & _ & neq_{A}_{B} & _ & _).\n"
    )
    w.write(
        f"pose proof (lemma_angledistinct _ _ _ _ _ _ CongA_X_Y_Z_x_{A}_{B}) as (_ & _ & _ & _ & neq_{A}_{B} & _).\n"
    )
    w.write(
        f"pose proof (lemma_angledistinct _ _ _ _ _ _ CongA_X_Y_Z_{A}_y_{B}) as (_ & _ & _ & _ & _ & neq_{A}_{B}).\n"
    )
    w.write("\n")
...
def lemma_betweennotequal(e: Hypothesis, stack: list[list[Any]], w: TextIO):
    ...
    for hh in hhs:
        ...
        if [A, B] == [Y, Z]:
            w.write(
                f"pose proof (lemma_betweennotequal _ _ _ BetS_{X}_{Y}_{Z}) as (neq_{Y}_{Z} & _ & _).\n"
            )
            break
        elif [A, B] == [X, Y]:
            w.write(
                f"pose proof (lemma_betweennotequal _ _ _ BetS_{X}_{Y}_{Z}) as (_ & neq_{X}_{Y} & _).\n"
            )
            break
        elif [A, B] == [X, Z]:
            w.write(
                f"pose proof (lemma_betweennotequal _ _ _ BetS_{X}_{Y}_{Z}) as (_ & _ & neq_{X}_{Z}).\n"
            )
            break
...
def lemma_collinear_ABC_ABD_BCD(e: Hypothesis, stack: list[list[Any]], w: TextIO):
    ...
    for hh_col in hhs_col:
        ...
        for hh_col2 in hhs_col:
            ...
            w.write(
                f"pose proof (lemma_collinear_ABC_ABD_BCD _ _ _ _ Col_{A}_{B}_{C} Col_{A}_{B}_{D} neq_{A}_{B}) as {e.to_var()}.\n"
            )
...
def lemma_congruenceflip(e: Hypothesis, stack: list[list[Any]], w: TextIO):
    ...
    B, A, D, C = e.points
    w.write(
        f"pose proof (lemma_congruenceflip {A} {B} {C} {D} Cong_{A}_{B}_{C}_{D}) as ({e.to_var()} & _ & _).\n"
    )
    B, A, C, D = e.points
    w.write(
        f"pose proof (lemma_congruenceflip {A} {B} {C} {D} Cong_{A}_{B}_{C}_{D}) as (_ & {e.to_var()} & _).\n"
    )
    A, B, D, C = e.points
    w.write(
        f"pose proof (lemma_congruenceflip {A} {B} {C} {D} Cong_{A}_{B}_{C}_{D}) as (_ & _ & {e.to_var()}).\n"
    )
    w.write("\n")
...
def lemma_doublereverse(e: Hypothesis, stack: list[list[Any]], w: TextIO):
    ...
    D, C, B, A = e.points
    w.write(
        f"pose proof (lemma_doublereverse {A} {B} {C} {D} Cong_{A}_{B}_{C}_{D}) as ({e.to_var()} & _).\n"
    )
    B, A, D, C = e.points
    w.write(
        f"pose proof (lemma_doublereverse {A} {B} {C} {D} Cong_{A}_{B}_{C}_{D}) as (_ & {e.to_var()}).\n"
    )
    w.write("\n")
...
def lemma_equalanglesNC(e: Hypothesis, stack: list[list[Any]], w: TextIO):
    a, b, c = e.points
    w.write(
        f"pose proof (lemma_equalanglesNC X Y Z {a} {b} {c} CongA_X_Y_Z_{a}_{b}_{c}) as {e.to_var()}.\n"
    )
...
def lemma_equalanglesreflexive(e: Hypothesis, stack: list[list[Any]], w: TextIO):
    A, B, C, A, B, C = e.points
    w.write(
        f"pose proof (lemma_equalanglesreflexive {A} {B} {C} nCol_{A}_{B}_{C}) as {e.to_var()}.\n"
    )
...
def lemma_equalanglessymmetric(e: Hypothesis, stack: list[list[Any]], w: TextIO):
    a, b, c, A, B, C = e.points
    w.write(
        f"pose proof (lemma_equalanglessymmetric {A} {B} {C} {a} {b} {c} CongA_{A}_{B}_{C}_{a}_{b}_{c}) as {e.to_var()}.\n"
    )
...
def Triangle(h: Hypothesis, stack: list[list[Any]], w: TextIO) -> None:
    ...
    if h.head == "nCol":
        w.write(
            f"pose proof (lemma_s_ncol_triangle {A} {B} {C} Triangle_{A}_{B}_{C}) as nCol_{A}_{B}_{C}.\n"
        )
        w.write(
            f"pose proof (lemma_s_ncol_n_col {A} {B} {C} nCol_{A}_{B}_{C}) as n_Col_{A}_{B}_{C}.\n"
        )
    elif h.head == "Triangle":
        w.write(
            f"pose proof (lemma_s_triangle {A} {B} {C} nCol_{A}_{B}_{C}) as Triangle_{A}_{B}_{C}.\n"
        )
    else:
        raise ValueError(
            f"Expected assertion by Triangle to be either nCol or Triangle, got: {h.head}"
        )
