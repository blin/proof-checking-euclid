...
class ConcludeAssertion(Assertion):
    ...
    def to_str(self) -> str:
        return f"\tpose proof ({self.by}) as {self.e.to_var()}.\n"
...
class DestructAssertion(Assertion):
    ...
    def to_str(self) -> str:
        ps = " & ".join(self.points)
        return f"\tpose proof ({self.by}) as ({ps} & {self.e.to_var()}).\n"
...
class CoqVisitor(NodeVisitor):
    ...
    def visit_assert_contradiction(self, node: Node, vc: list[Node]):
        h = vc[1]
        statements = [s[0] for s in vc[8]]
        return ContradictAssertion(e=h, by="contradiction", statements=statements)
