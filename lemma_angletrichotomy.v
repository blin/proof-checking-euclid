...
Proof.
	...
	assert (~ LtA D E F A B C) as n_LtA_D_E_F_A_B_C.
	{
		intros LtA_D_E_F_A_B_C.
		pose proof (lemma_angleordertransitive _ _ _ _ _ _ _ _ _ LtA_A_B_C_D_E_F LtA_D_E_F_A_B_C) as LtA_A_B_C_A_B_C.

		destruct LtA_A_B_C_A_B_C as (G & H & J & BetS_G_H_J & OnRay_B_A_G & OnRay_B_C_J & CongA_A_B_C_A_B_H).
		destruct CongA_A_B_C_A_B_H as (U & V & u & v & OnRay_B_A_U & OnRay_B_C_V & OnRay_B_A_u & OnRay_B_H_v & Cong_BU_Bu & Cong_BV_Bv & Cong_UV_uv & nCol_A_B_C).
		...
		pose proof (lemma_layoffunique _ _ _ _ OnRay_B_A_U OnRay_B_A_u Cong_BU_Bu) as eq_U_u.
		assert (Cong U V U v) as Cong_UV_Uv by (rewrite eq_U_u at 2; exact Cong_UV_uv).
		pose proof (lemma_onray_impliescollinear _ _ _ OnRay_B_A_U) as Col_B_A_U.
		pose proof (lemma_onray_impliescollinear _ _ _ OnRay_B_A_G) as Col_B_A_G.
		...
		pose proof (lemma_extension _ _ _ _ neq_H_G neq_H_G) as (P & BetS_H_G_P & Cong_GP_HG).
		...
		pose proof (lemma_onray_impliescollinear _ _ _ OnRay_B_C_J) as Col_B_C_J.
		pose proof (lemma_onray_strict _ _ _ OnRay_B_C_J) as neq_B_J.
		pose proof (lemma_inequalitysymmetric _ _ neq_B_J) as neq_J_B.

		assert (~ Col B A J) as n_Col_B_A_J.
		{
			intros Col_B_A_J.
			pose proof (lemma_collinearorder _ _ _ Col_B_A_J) as (_ & _ & Col_J_B_A & _ & _).
			pose proof (lemma_collinearorder _ _ _ Col_B_C_J) as (_ & _ & Col_J_B_C & _ & _).

			pose proof (lemma_collinear_ABC_ABD_BCD _ _ _ _ Col_J_B_A Col_J_B_C neq_J_B) as Col_B_A_C.
			pose proof (lemma_collinearorder _ _ _ Col_B_A_C) as (Col_A_B_C & _ & _ & _ & _).

			contradict Col_A_B_C.
			exact n_Col_A_B_C.
		}
		pose proof (lemma_s_n_col_ncol _ _ _ n_Col_B_A_J) as nCol_B_A_J.

		pose proof (lemma_s_os _ _ _ _ _ BetS_J_G_P Col_B_A_G nCol_B_A_J) as OS_J_B_A_P.
		...
		assert (~ Col B U H) as n_Col_B_U_H.
		{
			...
			pose proof (lemma_collinearorder _ _ _ Col_B_A_G) as (Col_A_B_G & _ & _ & _ & _).

			pose proof (lemma_collinearorder _ _ _ Col_B_A_H) as (Col_A_B_H & _ & _ & _ & _).
			...
			pose proof (lemma_collinearorder _ _ _ Col_B_A_H) as (_ & _ & Col_H_B_A & _ & _).
			...
			pose proof (lemma_collinearorder _ _ _ Col_G_B_J) as (Col_B_G_J & _ & _ & _ & _).

			pose proof (lemma_collinearorder _ _ _ Col_B_A_G) as (_ & _ & _ & Col_B_G_A & _).

			pose proof (lemma_onray_strict _ _ _ OnRay_B_A_G) as neq_B_G.
			pose proof (lemma_collinear_ABC_ABD_BCD _ _ _ _ Col_B_G_J Col_B_G_A neq_B_G) as Col_G_J_A.
			pose proof (lemma_collinearorder _ _ _ Col_G_B_J) as (_ & _ & _ & Col_G_J_B & _).
		...
		pose proof (lemma_onray_shared_initial_point _ _ _ _ OnRay_B_A_G OnRay_B_A_U) as OnRay_B_G_U.
		pose proof (lemma_onray_impliescollinear _ _ _ OnRay_B_G_U) as Col_B_G_U.
		pose proof (lemma_collinearorder _ _ _ Col_B_G_U) as (_ & _ & _ & Col_B_U_G & _).

		pose proof (lemma_s_os _ _ _ _ _ BetS_H_G_P Col_B_U_G nCol_B_U_H) as OS_H_B_U_P.

		assert (~ Col B U J) as n_Col_B_U_J.
		{
			intros Col_B_U_J.

			pose proof (lemma_collinearorder _ _ _ Col_B_C_J) as (_ & _ & _ & Col_B_J_C & _).

			pose proof (lemma_collinearorder _ _ _ Col_B_A_U) as (_ & _ & Col_U_B_A & _ & _).

			pose proof (lemma_collinearorder _ _ _ Col_B_U_J) as (Col_U_B_J & _ & _ & _ & _).

			pose proof (lemma_collinear_ABC_ABD_BCD _ _ _ _ Col_U_B_A Col_U_B_J neq_U_B) as Col_B_A_J.
			pose proof (lemma_collinearorder _ _ _ Col_B_C_J) as (_ & _ & Col_J_B_C & _ & _).

			pose proof (lemma_collinearorder _ _ _ Col_B_A_J) as (_ & _ & Col_J_B_A & _ & _).

			pose proof (lemma_collinear_ABC_ABD_BCD _ _ _ _ Col_J_B_C Col_J_B_A neq_J_B) as Col_B_C_A.
			pose proof (lemma_collinearorder _ _ _ Col_B_C_A) as (_ & _ & Col_A_B_C & _ & _).

			contradict Col_A_B_C.
			exact n_Col_A_B_C.
		}
		pose proof (lemma_s_n_col_ncol _ _ _ n_Col_B_U_J) as nCol_B_U_J.

		pose proof (lemma_s_ss _ _ _ _ _ _ _ Col_B_U_G Col_B_U_G BetS_J_G_P BetS_H_G_P nCol_B_U_J nCol_B_U_H) as SS_J_H_B_U.
		pose proof (lemma_samesidesymmetric _ _ _ _ SS_J_H_B_U) as (SS_H_J_B_U & _ & _).
		pose proof (lemma_onray_shared_initial_point _ _ _ _ OnRay_B_C_J OnRay_B_C_V) as OnRay_B_J_V.
		assert (eq B B) as eq_B_B by (reflexivity).
		assert (Col B B U) as Col_B_B_U by (unfold Col; one_of_disjunct eq_B_B).
		pose proof (lemma_sameside2 _ _ _ _ _ _ SS_H_J_B_U Col_B_B_U OnRay_B_J_V) as SS_H_V_B_U.
		pose proof (lemma_samesidesymmetric _ _ _ _ SS_H_V_B_U) as (SS_V_H_B_U & _ & _).
		pose proof (lemma_sameside2 _ _ _ _ _ _ SS_V_H_B_U Col_B_B_U OnRay_B_H_v) as SS_V_v_B_U.
		...
		pose proof (proposition_07 B _ V v neq_B_U Cong_VB_vB Cong_VU_vU SS_V_v_B_U) as eq_V_v.
		assert (OnRay B H V) as OnRay_B_H_V by (rewrite eq_V_v; exact OnRay_B_H_v).
		pose proof (lemma_onray_impliescollinear _ _ _ OnRay_B_H_V) as Col_B_H_V.
		pose proof (lemma_onray_impliescollinear _ _ _ OnRay_B_J_V) as Col_B_J_V.
		...
		pose proof (lemma_onray_strict _ _ _ OnRay_B_H_V) as neq_B_V.
		...
		pose proof (lemma_collinearorder _ _ _ Col_B_A_G) as (_ & _ & Col_G_B_A & _ & _).
