...
Proof.
	...
	pose proof (by_prop_BetS_notequal _ _ _ BetS_B_D_C) as (neq_D_C & _ & _).
	...
	pose proof (by_prop_Col_order _ _ _ Col_B_D_C) as (Col_D_B_C & Col_D_C_B & Col_C_B_D & Col_B_C_D & Col_C_D_B).

	pose proof (by_prop_nCol_distinct _ _ _ nCol_B_C_A) as (neq_B_C & neq_C_A & neq_B_A & neq_C_B & neq_A_C & neq_A_B).
	pose proof (by_prop_nCol_order _ _ _ nCol_B_C_A) as (nCol_C_B_A & nCol_C_A_B & nCol_A_B_C & nCol_B_A_C & nCol_A_C_B).

	pose proof (lemma_s_ncol_ABD_col_ABC_ncol_ACD _ _ _ _ nCol_B_C_A Col_B_C_D neq_B_D) as nCol_B_D_A.
	pose proof (by_prop_nCol_distinct _ _ _ nCol_B_D_A) as (_ & neq_D_A & _ & _ & neq_A_D & _).
	pose proof (by_prop_nCol_order _ _ _ nCol_B_D_A) as (nCol_D_B_A & nCol_D_A_B & nCol_A_B_D & nCol_B_A_D & nCol_A_D_B).

	pose proof (by_prop_CongA_ABCequalsCBA _ _ _ nCol_B_D_A) as CongA_BDA_ADB.
	...
	pose proof (by_prop_BetS_notequal _ _ _ BetS_A_M_D) as (_ & neq_A_M & _).
	pose proof (by_prop_BetS_notequal _ _ _ BetS_A_M_D) as (neq_M_D & _ & _).

	pose proof (by_def_Col_from_BetS_A_B_C _ _ _ BetS_A_M_D) as Col_A_M_D.
	pose proof (by_prop_Col_order _ _ _ Col_A_M_D) as (Col_M_A_D & Col_M_D_A & Col_D_A_M & Col_A_D_M & Col_D_M_A).

	pose proof (by_prop_Cong_symmetric _ _ _ _ Cong_MA_MD) as Cong_MD_MA.
	pose proof (by_prop_Cong_flip _ _ _ _ Cong_MD_MA) as (_ & Cong_DM_MA & _).
	pose proof (by_prop_Cong_flip _ _ _ _ Cong_MA_MD) as (_ & Cong_AM_MD & _).

	pose proof (by_prop_nCol_helper _ _ _ _ _ nCol_B_D_A Col_B_D_C Col_B_D_D neq_C_D) as nCol_C_D_A.
	pose proof (by_prop_nCol_order _ _ _ nCol_C_D_A) as (nCol_D_C_A & nCol_D_A_C & nCol_A_C_D & nCol_C_A_D & nCol_A_D_C).
	pose proof (by_prop_CongA_ABCequalsCBA _ _ _ nCol_C_D_A) as CongA_CDA_ADC.

	pose proof (by_prop_nCol_helper _ _ _ _ _ nCol_A_D_C Col_A_D_A Col_A_D_M neq_A_M) as nCol_A_M_C.
	pose proof (by_prop_nCol_distinct _ _ _ nCol_A_M_C) as (_ & neq_M_C & _ & _ & neq_C_M & _).
	pose proof (by_prop_nCol_order _ _ _ nCol_A_M_C) as (nCol_M_A_C & nCol_M_C_A & nCol_C_A_M & nCol_A_C_M & nCol_C_M_A).

	pose proof (by_prop_nCol_helper _ _ _ _ _ nCol_A_D_B Col_A_D_A Col_A_D_M neq_A_M) as nCol_A_M_B.
	pose proof (by_prop_nCol_distinct _ _ _ nCol_A_M_B) as (_ & neq_M_B & _ & _ & neq_B_M & _).
	pose proof (by_prop_nCol_order _ _ _ nCol_A_M_B) as (nCol_M_A_B & nCol_M_B_A & nCol_B_A_M & nCol_A_B_M & nCol_B_M_A).
	...
	pose proof (by_prop_Cong_symmetric _ _ _ _ Cong_ME_MC) as Cong_MC_ME.
	pose proof (by_prop_Cong_flip _ _ _ _ Cong_MC_ME) as (_ & Cong_CM_ME & _).
	pose proof (by_prop_Cong_flip _ _ _ _ Cong_CM_ME) as (Cong_MC_EM & _ & _).
	pose proof (by_prop_Cong_symmetric _ _ _ _ Cong_MC_EM) as Cong_EM_MC.
	...
	pose proof (by_prop_BetS_notequal _ _ _ BetS_B_M_F) as (neq_M_F & _ & _).
	...
	pose proof (by_prop_Col_order _ _ _ Col_B_M_F) as (Col_M_B_F & Col_M_F_B & Col_F_B_M & Col_B_F_M & Col_F_M_B).

	pose proof (by_prop_Cong_flip _ _ _ _ Cong_MF_MB) as (_ & _ & Cong_MF_BM).
	pose proof (by_prop_Cong_symmetric _ _ _ _ Cong_MF_BM) as Cong_BM_MF.

	pose proof (by_def_Midpoint _ _ _ BetS_B_M_F Cong_BM_MF) as Midpoint_B_M_F.

	pose proof (by_prop_nCol_helper _ _ _ _ _ nCol_B_M_A Col_B_M_F Col_B_M_M neq_F_M) as nCol_F_M_A.
	pose proof (by_prop_nCol_order _ _ _ nCol_F_M_A) as (nCol_M_F_A & nCol_M_A_F & nCol_A_F_M & nCol_F_A_M & nCol_A_M_F).

	pose proof (by_prop_nCol_helper _ _ _ _ _ nCol_A_M_F Col_A_M_A Col_A_M_D neq_A_D) as nCol_A_D_F.
	pose proof (by_prop_nCol_order _ _ _ nCol_A_D_F) as (nCol_D_A_F & nCol_D_F_A & nCol_F_A_D & nCol_A_F_D & nCol_F_D_A).

	pose proof (by_prop_CongA_ABCequalsCBA _ _ _ nCol_F_A_D) as CongA_FAD_DAF.
	...
	pose proof (by_prop_Cong_flip _ _ _ _ Cong_BD_FA) as (Cong_DB_AF & _ & _).
	pose proof (by_prop_Cong_symmetric _ _ _ _ Cong_DB_AF) as Cong_AF_DB.
	pose proof (by_prop_Cong_flip _ _ _ _ Cong_DB_AF) as (_ & Cong_BD_AF & _).
	pose proof (by_prop_Cong_symmetric _ _ _ _ Cong_BD_AF) as Cong_AF_BD.

	pose proof (by_prop_Cong_flip _ _ _ _ Cong_DC_AE) as (_ & _ & Cong_DC_EA).
	pose proof (by_prop_Cong_symmetric _ _ _ _ Cong_DC_EA) as Cong_EA_DC.

	pose proof (by_prop_Cong_symmetric _ _ _ _ Cong_BA_FD) as Cong_FD_BA.
	...
	pose proof (by_prop_BetS_notequal _ _ _ BetS_E_A_F) as (neq_A_F & _ & _).
	pose proof (by_prop_BetS_notequal _ _ _ BetS_E_A_F) as (_ & neq_E_A & _).
	pose proof (by_prop_neq_symmetric _ _ neq_E_A) as neq_A_E.

	pose proof (by_def_Col_from_BetS_A_B_C _ _ _ BetS_E_A_F) as Col_E_A_F.
	pose proof (by_prop_Col_order _ _ _ Col_E_A_F) as (Col_A_E_F & Col_A_F_E & Col_F_E_A & Col_E_F_A & Col_F_A_E).

	pose proof (lemma_s_ncol_ABD_col_ABC_ncol_ACD _ _ _ _ nCol_A_F_D Col_A_F_E neq_A_E) as nCol_A_E_D.
	pose proof (by_prop_nCol_distinct _ _ _ nCol_A_E_D) as (_ & neq_E_D & _ & _ & neq_D_E & _).
	pose proof (by_prop_nCol_order _ _ _ nCol_A_E_D) as (nCol_E_A_D & nCol_E_D_A & nCol_D_A_E & nCol_A_D_E & nCol_D_E_A).
	pose proof (by_prop_CongA_ABCequalsCBA _ _ _ nCol_D_A_E) as CongA_DAE_EAD.
	...
	pose proof (by_prop_CongA_transitive _ _ _ _ _ _ _ _ _ CongA_FAD_BDA CongA_BDA_ADB) as CongA_FAD_ADB.
	pose proof (by_prop_CongA_symmetric _ _ _ _ _ _ CongA_FAD_ADB) as CongA_ADB_FAD.
	pose proof (by_prop_CongA_transitive _ _ _ _ _ _ _ _ _ CongA_ADB_FAD CongA_FAD_DAF) as CongA_ADB_DAF.
	pose proof (by_prop_CongA_symmetric _ _ _ _ _ _ CongA_ADB_DAF) as CongA_DAF_ADB.
	pose proof (by_prop_CongA_transitive _ _ _ _ _ _ _ _ _ CongA_DAF_ADB CongA_ADB_BDA) as CongA_DAF_BDA.

	pose proof (by_prop_CongA_transitive _ _ _ _ _ _ _ _ _ CongA_EAD_CDA CongA_CDA_ADC) as CongA_EAD_ADC.
	pose proof (by_prop_CongA_transitive _ _ _ _ _ _ _ _ _ CongA_DAE_EAD CongA_EAD_CDA) as CongA_DAE_CDA.
	...
	pose proof (by_prop_Par_flip _ _ _ _ Par_FE_CB) as (_ & _ & Par_EF_BC).
