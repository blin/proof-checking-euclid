...
Proof.
	...
	pose proof (by_prop_Cong_symmetric _ _ _ _ Cong_AB_DB) as Cong_DB_AB.
	pose proof (by_prop_Cong_flip _ _ _ _ Cong_AC_DC) as (Cong_CA_CD & _ & _).
	pose proof (by_prop_Cong_flip _ _ _ _ Cong_AB_DB) as (Cong_BA_BD & _ & _).

	assert (RightTriangle_ABH2 := RightTriangle_ABH).
	destruct RightTriangle_ABH2 as (F & BetS_A_B_F & Cong_AB_FB & Cong_AH_FH & _).

	pose proof (by_prop_Cong_transitive _ _ _ _ _ _ Cong_DB_AB Cong_AB_FB) as Cong_DB_FB.
	pose proof (by_prop_Cong_doublereverse _ _ _ _ Cong_DB_FB) as (Cong_BF_BD & _).

	pose proof (by_prop_Cong_doublereverse _ _ _ _ Cong_BC_BH) as (_ & Cong_CB_HB).
	pose proof (by_prop_Cong_flip _ _ _ _ Cong_BC_BH) as (_ & Cong_CB_BH & _).
	...
	pose proof (by_prop_Cong_flip _ _ _ _ Cong_AH_DH) as (Cong_HA_HD & _ & _).
	pose proof (by_prop_Cong_symmetric _ _ _ _ Cong_AH_DH) as Cong_DH_AH.
	pose proof (by_prop_Cong_flip _ _ _ _ Cong_DH_AH) as (Cong_HD_HA & _ & _).
	...
	pose proof (by_prop_Cong_flip _ _ _ _ Cong_MC_MH) as (Cong_CM_HM & _ & _).
	...
	assert (eq B M \/ neq B M) as [eq_B_M|neq_B_M] by (apply Classical_Prop.classic).
	{
		...
		pose proof (by_prop_RightTriangle_reverse _ _ _ _ RightTriangle_CBA BetS_C_B_H Cong_CB_BH) as Cong_CA_HA.
		pose proof (by_prop_Cong_flip _ _ _ _ Cong_CA_HA) as (Cong_AC_AH & _ & _).
	...
	pose proof (by_prop_RightTriangle_symmetric _ _ _ RightTriangle_CMB) as RightTriangle_BMC.
	...
	*)
	...
	pose proof (by_prop_Cong_flip _ _ _ _ Cong_MA_MD) as (Cong_AM_DM & _ & _).

	pose proof (by_def_RightTriangle _ _ _ _ BetS_A_B_D Cong_AB_DB Cong_AM_DM neq_B_M) as RightTriangle_ABM.
	pose proof (by_prop_RightTriangle_symmetric _ _ _ RightTriangle_ABM) as RightTriangle_MBA.
	...
	pose proof (by_prop_Cong_flip _ _ _ _ Cong_CA_HD) as (Cong_AC_DH & _ & _).
	pose proof (by_prop_Cong_transitive _ _ _ _ _ _ Cong_CA_HD Cong_HD_HA) as Cong_CA_HA.
	pose proof (by_prop_Cong_flip _ _ _ _ Cong_CA_HA) as (Cong_AC_AH & _ & _).
