...
Proof.
	...
	apply by_prop_Cong_symmetric in Cong_BD_DA as Cong_DA_BD.
	apply by_prop_Cong_flip in Cong_DA_BD as (_ & _ & Cong_DA_DB).
	assert (nCol_A_B_D := Triangle_ABD).
	unfold Triangle in nCol_A_B_D.

	pose proof (postulate_Euclid3 _ _ neq_B_C) as (J & CI_J_B_BC).
	apply by_prop_nCol_distinct in nCol_A_B_D as (
	...
	pose proof(by_prop_BetS_notequal _ _ _ BetS_D_B_G) as (neq_B_G & _ & neq_D_G).
	...
	apply by_prop_Cong_symmetric in Cong_DA_DB as Cong_DB_DA.
	apply by_prop_Cong_symmetric in Cong_DL_DG as Cong_DG_DL.
	pose proof (
