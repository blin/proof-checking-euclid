...
Proof.
	...
	pose proof (by_prop_Par_symmetric _ _ _ _ Par_AB_EF) as Par_EF_AB.
	pose proof (by_prop_Par_flip _ _ _ _ Par_EF_AB) as (_ & Par_EF_BA & _).
	pose proof (by_prop_Par_flip _ _ _ _ Par_AB_EF) as (_ & Par_AB_FE & _).
	pose proof (by_prop_Par_NC _ _ _ _ Par_AB_EF) as (nCol_A_B_E & _ & _ & _).
	pose proof (by_prop_nCol_distinct _ _ _ nCol_A_B_E) as (_ & _ & _ & neq_B_A & _ & _).

	pose proof (by_prop_Par_flip _ _ _ _ Par_CD_EF) as (_ & Par_CD_FE & _).
	pose proof (by_prop_Par_symmetric _ _ _ _ Par_CD_EF) as Par_EF_CD.
	pose proof (by_prop_Par_flip _ _ _ _ Par_EF_CD) as (_ & Par_EF_DC & _).
	pose proof (by_prop_Par_NC _ _ _ _ Par_CD_EF) as (nCol_C_D_E & _ & _ & _).
	pose proof (by_prop_nCol_distinct _ _ _ nCol_C_D_E) as (neq_C_D & _ & _ & _ & _ & _).
	...
	pose proof (by_prop_Col_order _ _ _ Col_A_B_G) as (_ & _ & Col_G_A_B & _ & _).
	pose proof (by_prop_Col_order _ _ _ Col_A_B_G) as (Col_B_A_G & _ & _ & _ & _).

	pose proof (by_prop_Col_order _ _ _ Col_E_F_H) as (Col_F_E_H & _ & _ & _ & _).
	pose proof (by_prop_Col_order _ _ _ Col_E_F_H) as (_ & _ & Col_H_E_F & _ & _).

	pose proof (by_prop_Col_order _ _ _ Col_C_D_K) as (Col_D_C_K & _ & _ & _ & _).

	pose proof (by_prop_neq_symmetric _ _ neq_A_G) as neq_G_A.
	pose proof (by_prop_neq_symmetric _ _ neq_E_H) as neq_H_E.
	pose proof (by_prop_neq_symmetric _ _ neq_C_K) as neq_K_C.

	pose proof (by_prop_Par_collinear _ _ _ _ _ Par_AB_FE Col_F_E_H neq_H_E) as Par_AB_HE.
	pose proof (by_prop_Par_collinear _ _ _ _ _ Par_CD_FE Col_F_E_H neq_H_E) as Par_CD_HE.

	pose proof (by_prop_Par_flip _ _ _ _ Par_AB_HE) as (_ & Par_AB_EH & _).
	pose proof (by_prop_Par_symmetric _ _ _ _ Par_AB_EH) as Par_EH_AB.
	pose proof (by_prop_Par_flip _ _ _ _ Par_EH_AB) as (_ & Par_EH_BA & _).
	pose proof (by_prop_Par_flip _ _ _ _ Par_CD_HE) as (_ & Par_CD_EH & _).
	pose proof (by_prop_Par_symmetric _ _ _ _ Par_CD_EH) as Par_EH_CD.
	pose proof (by_prop_Par_flip _ _ _ _ Par_EH_CD) as (_ & Par_EH_DC & _).
	...
	pose proof (by_prop_Col_order _ _ _ Col_G_A_b) as (_ & Col_A_b_G & _ & _ & _).
	pose proof (by_prop_Col_order _ _ _ Col_A_b_G) as (Col_b_A_G & _ & _ & _ & _).

	pose proof (by_prop_Col_ABC_ABD_BCD _ _ _ _ Col_G_A_b Col_G_A_B neq_G_A) as Col_A_b_B.
	pose proof (by_prop_Col_order _ _ _ Col_A_b_B) as (_ & _ & Col_B_A_b & _ & _).
	pose proof (by_prop_Col_order _ _ _ Col_A_b_B) as (Col_b_A_B & _ & _ & _ & _).

	pose proof (axiom_betweennesssymmetry _ _ _ BetS_E_H_f) as BetS_f_H_E.
	pose proof (by_prop_BetS_notequal _ _ _ BetS_E_H_f) as (_ & _ & neq_E_f).
	pose proof (by_prop_BetS_notequal _ _ _ BetS_E_H_f) as (neq_H_f & _ & _).
	...
	pose proof (by_prop_Col_order _ _ _ Col_E_H_f) as (Col_H_E_f & _ & _ & _ & _).
	pose proof (by_prop_Col_order _ _ _ Col_H_E_f) as (_ & Col_E_f_H & _ & _ & _).
	pose proof (by_prop_Col_order _ _ _ Col_E_f_H) as (Col_f_E_H & _ & _ & _ & _).
	pose proof (by_prop_Col_order _ _ _ Col_E_f_H) as (_ & Col_f_H_E & _ & _ & _).

	pose proof (by_prop_Col_ABC_ABD_BCD _ _ _ _ Col_H_E_f Col_H_E_F neq_H_E) as Col_E_f_F.
	pose proof (by_prop_Col_order _ _ _ Col_E_f_F) as (_ & _ & Col_F_E_f & _ & _).

	pose proof (axiom_betweennesssymmetry _ _ _ BetS_C_K_d) as BetS_d_K_C.
	pose proof (by_prop_BetS_notequal _ _ _ BetS_C_K_d) as (_ & _ & neq_C_d).
	pose proof (by_prop_BetS_notequal _ _ _ BetS_C_K_d) as (neq_K_d & _ & _).
	pose proof (by_prop_neq_symmetric _ _ neq_C_d) as neq_d_C.
	pose proof (by_prop_neq_symmetric _ _ neq_K_d) as neq_d_K.

	pose proof (by_def_Col_from_BetS_A_B_C _ _ _ BetS_C_K_d) as Col_C_K_d.
	pose proof (by_prop_Col_order _ _ _ Col_C_K_d) as (Col_K_C_d & _ & _ & _ & _).
	pose proof (by_prop_Col_order _ _ _ Col_C_K_d) as (_ & _ & Col_d_C_K & _ & _).
	pose proof (by_prop_Col_order _ _ _ Col_C_D_K) as (_ & _ & Col_K_C_D & _ & _).

	pose proof (by_prop_Col_ABC_ABD_BCD _ _ _ _ Col_K_C_d Col_K_C_D neq_K_C) as Col_C_d_D.
	pose proof (by_prop_Col_order _ _ _ Col_C_d_D) as (Col_d_C_D & _ & _ & _ & _).
	pose proof (by_prop_Col_order _ _ _ Col_C_d_D) as (_ & _ & Col_D_C_d & _ & _).

	pose proof (by_prop_Par_collinear _ _ _ _ _ Par_EF_DC Col_D_C_d neq_d_C) as Par_EF_dC.
	pose proof (by_prop_Par_collinear _ _ _ _ _ Par_EF_BA Col_B_A_b neq_b_A) as Par_EF_bA.
	pose proof (by_prop_Par_flip _ _ _ _ Par_EF_bA) as (_ & Par_EF_Ab & _).
	pose proof (by_prop_Par_flip _ _ _ _ Par_EF_dC) as (_ & Par_EF_Cd & _).
	pose proof (by_prop_Par_symmetric _ _ _ _ Par_EF_Ab) as Par_Ab_EF.
	pose proof (by_prop_Par_symmetric _ _ _ _ Par_EF_Cd) as Par_Cd_EF.
	pose proof (by_prop_Par_flip _ _ _ _ Par_Ab_EF) as (_ & Par_Ab_FE & _).
	pose proof (by_prop_Par_flip _ _ _ _ Par_Cd_EF) as (_ & Par_Cd_FE & _).

	pose proof (by_prop_Par_collinear _ _ _ _ _ Par_Ab_FE Col_F_E_f neq_f_E) as Par_Ab_fE.
	pose proof (by_prop_Par_collinear _ _ _ _ _ Par_Cd_FE Col_F_E_f neq_f_E) as Par_Cd_fE.
	pose proof (by_prop_Par_flip _ _ _ _ Par_Ab_fE) as (_ & Par_Ab_Ef & _).
	pose proof (by_prop_Par_flip _ _ _ _ Par_Cd_fE) as (_ & Par_Cd_Ef & _).
	pose proof (by_prop_Par_symmetric _ _ _ _ Par_Cd_fE) as Par_fE_Cd.
	pose proof (by_prop_Par_symmetric _ _ _ _ Par_Cd_Ef) as Par_Ef_Cd.
	pose proof (by_prop_Par_flip _ _ _ _ Par_Cd_Ef) as (Par_dC_Ef & _ & _).
	pose proof (by_prop_Par_flip _ _ _ _ Par_Cd_fE) as (Par_dC_fE & _ & _).

	pose proof (parnotmeet _ _ _ _ Par_Ef_Cd) as n_Meet_E_f_C_d.
	pose proof (parnotmeet _ _ _ _ Par_fE_Cd) as n_Meet_f_E_C_d.

	pose proof (by_prop_Par_collinear _ _ _ _ _ Par_Ab_fE Col_f_E_H neq_H_E) as Par_Ab_HE.
	pose proof (by_prop_Par_symmetric _ _ _ _ Par_Ab_HE) as Par_HE_Ab.
	pose proof (by_prop_Par_flip _ _ _ _ Par_HE_Ab) as (_ & _ & Par_EH_bA).
	pose proof (by_prop_Par_collinear _ _ _ _ _ Par_EH_bA Col_b_A_G neq_G_A) as Par_EH_GA.

	pose proof (by_prop_Par_flip _ _ _ _ Par_EH_GA) as (_ & Par_EH_AG & _).
	pose proof (by_prop_Par_symmetric _ _ _ _ Par_EH_AG) as Par_AG_EH.
	pose proof (by_prop_Par_NC _ _ _ _ Par_AG_EH) as (_ & _ & _ & nCol_A_G_H).

	pose proof (by_prop_Par_collinear _ _ _ _ _ Par_Cd_fE Col_f_E_H neq_H_E) as Par_Cd_HE.
	pose proof (by_prop_Par_symmetric _ _ _ _ Par_Cd_HE) as Par_HE_Cd.
	pose proof (by_prop_Par_flip _ _ _ _ Par_HE_Cd) as (_ & Par_HE_dC & _).
	pose proof (by_prop_Par_collinear _ _ _ _ _ Par_HE_dC Col_d_C_K neq_K_C) as Par_HE_KC.
	pose proof (by_prop_Par_flip _ _ _ _ Par_HE_KC) as (_ & _ & Par_EH_CK).

	pose proof (by_prop_Par_NC _ _ _ _ Par_EH_CK) as (_ & _ & _ & nCol_E_H_K).
	pose proof (by_prop_nCol_order _ _ _ nCol_E_H_K) as (_ & _ & _ & _ & nCol_K_H_E).
	pose proof (by_prop_Par_collinear _ _ _ _ _ Par_EH_DC Col_D_C_K neq_K_C) as Par_EH_KC.
	pose proof (by_prop_Par_symmetric _ _ _ _ Par_EH_CK) as Par_CK_EH.
	pose proof (by_prop_Par_NC _ _ _ _ Par_CK_EH) as (_ & _ & _ & nCol_C_K_H).
	pose proof (by_prop_nCol_order _ _ _ nCol_C_K_H) as (_ & nCol_K_H_C & _ & _ & _).
	pose proof (by_prop_nCol_helper _ _ _ _ _ nCol_E_H_K Col_E_H_f Col_E_H_H neq_f_H) as nCol_f_H_K.
	pose proof (by_prop_nCol_order _ _ _ nCol_f_H_K) as (_ & _ & _ & _ & nCol_K_H_f).
	pose proof (by_prop_nCol_helper _ _ _ _ _ nCol_C_K_H Col_C_K_d Col_C_K_K neq_d_K) as nCol_d_K_H.
	...
	destruct Cross_Af_GH_or_Cross_AE_GH as [Cross_Af_GH | Cross_AE_GH].
	{
		...
		}
		{
			...
			pose proof (by_prop_Col_order _ _ _ Col_K_M_H) as (_ & _ & _ & Col_K_H_M & _).
			...
			pose proof (by_prop_Col_order _ _ _ Col_K_H_m) as (Col_H_K_m & _ & _ & _ & _).
			...
			pose proof (by_prop_OppositeSide_symmetric _ _ _ _ OppositeSide_d_HK_f) as OppositeSide_f_HK_d.
			...
			pose proof (by_prop_Par_flip _ _ _ _ Par_Ab_dC) as (_ & Par_Ab_Cd & _).
	...
	}
	{
		...
		destruct Cross_Cf_KH_or_Cross_CE_KH as [Cross_Cf_KH | Cross_CE_KH].
		{
			...
			pose proof (by_prop_OppositeSide_symmetric _ _ _ _ OppositeSide_d_HK_E) as OppositeSide_E_HK_d.
		...
		}
		{
			...
			pose proof (lemma_crossimpliesopposite _ _ _ _ Cross_AE_GH nCol_A_G_H) as (OppositeSide_A_GH_E & _ & _ & _).
			pose proof (by_prop_OppositeSide_symmetric _ _ _ _ OppositeSide_C_HK_E) as OppositeSide_E_HK_C.
	...
	pose proof (by_prop_Par_collinear _ _ _ _ _ Par_Ab_dC Col_d_C_D neq_D_C) as Par_Ab_DC.
	pose proof (by_prop_Par_flip _ _ _ _ Par_Ab_DC) as (_ & Par_Ab_CD & _).
	...
	pose proof (by_prop_Par_collinear _ _ _ _ _ Par_CD_bA Col_b_A_B neq_B_A) as Par_CD_BA.
