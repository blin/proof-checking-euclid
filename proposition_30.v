...
Proof.
	...
	pose proof (lemma_paralleldef2B _ _ _ _ Par_E_H_C_K) as TP_E_H_C_K.

	destruct TP_E_H_C_K as (_ & _ & _ & SS_C_K_E_H).
	...
	pose proof (lemma_parallelsymmetric _ _ _ _ Par_C_d_E_f) as Par_E_f_C_d.
	pose proof (lemma_paralleldef2B _ _ _ _ Par_E_f_C_d) as TP_E_f_C_d.

	destruct TP_E_f_C_d as (_ & _ & _ & SS_C_d_E_f).
	...
	(* assert (~ ~ CR A f G H \/ CR A E G H) as n_n_CR_A_f_G_H | CR_A_E_G_H. *)
	assert (~ ~ (CR A f G H \/ CR A E G H)) as n_n_CR_A_f_G_H_or_CR_A_E_G_H.
	{
		intro n_CR_A_f_G_H_and_n_CR_A_E_G_H.

		apply Classical_Prop.not_or_and in n_CR_A_f_G_H_and_n_CR_A_E_G_H.
		destruct n_CR_A_f_G_H_and_n_CR_A_E_G_H as (n_CR_A_f_G_H & n_CR_A_E_G_H).

		pose proof (lemma_30helper _ _ _ _ _ _ Par_A_b_E_f BetS_A_G_b BetS_E_H_f n_CR_A_f_G_H) as CR_A_E_G_H.

		contradict CR_A_E_G_H.
		exact n_CR_A_E_G_H.
	}
	assert (CR_A_f_G_H_or_CR_A_E_G_H := n_n_CR_A_f_G_H_or_CR_A_E_G_H).
	apply Classical_Prop.NNPP in CR_A_f_G_H_or_CR_A_E_G_H.


	assert (~ ~ (CR C f K H \/ CR C E K H)) as n_n_CR_C_f_K_H_or_CR_C_E_K_H.
	{
		intro n_CR_C_f_K_H_and_n_CR_C_E_K_H.

		apply Classical_Prop.not_or_and in n_CR_C_f_K_H_and_n_CR_C_E_K_H.
		destruct n_CR_C_f_K_H_and_n_CR_C_E_K_H as (n_CR_C_f_K_H & n_CR_C_E_K_H).

		pose proof (lemma_30helper _ _ _ _ _ _ Par_C_d_E_f BetS_C_K_d BetS_E_H_f n_CR_C_f_K_H) as CR_C_E_K_H.

		contradict CR_C_E_K_H.
		exact n_CR_C_E_K_H.
	}
	assert (CR_C_f_K_H_or_CR_C_E_K_H := n_n_CR_C_f_K_H_or_CR_C_E_K_H).
	apply Classical_Prop.NNPP in CR_C_f_K_H_or_CR_C_E_K_H.
	...
	destruct CR_A_f_G_H_or_CR_A_E_G_H as [CR_A_f_G_H | CR_A_E_G_H].
	{
		(* case CR_A_f_G_H *)
		pose proof (lemma_crossimpliesopposite _ _ _ _ CR_A_f_G_H nCol_A_G_H) as (OS_A_G_H_f & _ & _ & _).

		(* assert by cases *)
		assert (Par A b C d) as Par_A_b_C_d.
		destruct CR_C_f_K_H_or_CR_C_E_K_H as [CR_C_f_K_H | CR_C_E_K_H].
		{
			(* case CR_C_f_K_H *)
			pose proof (lemma_crossimpliesopposite _ _ _ _ CR_C_f_K_H nCol_C_K_H) as (_ & _ & _ & OS_f_H_K_C).
		...
		}
		{
			...
			destruct CR_C_E_K_H as (M & BetS_C_M_E & BetS_K_M_H).
			...
			assert (CR d f K H) as CR_d_f_K_H.
			unfold CR.
			exists m.
			split.
			exact BetS_d_m_f.
			...
			pose proof (lemma_crossimpliesopposite _ _ _ _ CR_d_f_K_H nCol_d_K_H) as (_ & OS_d_H_K_f & _ & _).
	...
	}
	{
		...
		destruct CR_C_f_K_H_or_CR_C_E_K_H as [CR_C_f_K_H | CR_C_E_K_H].
		{
			...
			destruct CR_C_f_K_H as (M & BetS_C_M_f & BetS_K_M_H).
			pose proof (lemma_s_col_BetS_A_B_C K M H BetS_K_M_H) as Col_K_M_H.
			pose proof (lemma_collinearorder _ _ _ Col_K_M_H) as (_ & _ & _ & Col_K_H_M & _).
			...
			assert (CR d E K H) as CR_d_E_K_H.
			unfold CR.
			exists m.
			split.
			exact BetS_d_m_E.
			...
			pose proof (lemma_s_col_eq_B_C C K K eq_K_K) as Col_C_K_K.
			pose proof (lemma_NChelper _ _ _ _ _ nCol_C_K_H Col_C_K_d Col_C_K_K neq_d_K) as nCol_d_K_H.
			pose proof (lemma_crossimpliesopposite _ _ _ _ CR_d_E_K_H nCol_d_K_H) as (_ & OS_d_H_K_E & _ & _).
			...
			pose proof (lemma_crossimpliesopposite _ _ _ _ CR_A_E_G_H nCol_A_G_H) as (OS_A_G_H_E & _ & _ & _).
			pose proof (axiom_betweennesssymmetry _ _ _ BetS_E_H_f) as BetS_f_H_E.
		...
		{
			(* case CR_C_E_K_H *)
			pose proof (lemma_crossimpliesopposite _ _ _ _ CR_C_E_K_H nCol_C_K_H) as (_ & OS_C_H_K_E & _ & _).
