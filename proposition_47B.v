...
Proof.
	...
	pose proof (by_prop_CongA_reflexive _ _ _ nCol_A_B_C) as CongA_A_B_C_A_B_C.
	pose proof (by_prop_CongA_ABCequalsCBA _ _ _ nCol_A_B_C) as CongA_A_B_C_C_B_A.
	pose proof (by_prop_CongA_reflexive _ _ _ nCol_C_B_A) as CongA_C_B_A_C_B_A.
	...
	pose proof (proposition_34 _ _ _ _ Parallelogram_B_A_G_F) as (_ & _ & _ & _ & CongTriangles_A_B_F_F_G_A).
	pose proof (axiom_congruentequal _ _ _ _ _ _ CongTriangles_A_B_F_F_G_A) as EqAreaTri_A_B_F_F_G_A.
	...
	destruct Parallelogram_A_B_F_G_2 as (Par_A_B_F_G & Par_A_G_B_F).

	pose proof (by_prop_Par_flip _ _ _ _ Par_A_B_F_G) as (_ & Par_A_B_G_F & _).
	pose proof (by_prop_Par_to_TarskiPar _ _ _ _ Par_A_B_G_F) as TarskiPar_A_B_G_F.
	assert (TarskiPar_A_B_G_F_2 := TarskiPar_A_B_G_F).
	destruct TarskiPar_A_B_G_F_2 as (_ & _ & _ & SameSide_G_F_A_B).
	pose proof (by_prop_SameSide_symmetric _ _ _ _ SameSide_G_F_A_B) as (SameSide_F_G_A_B & _ & _).

	pose proof (by_prop_Par_flip _ _ _ _ Par_A_G_B_F) as (_ & Par_A_G_F_B & _).
	pose proof (by_prop_Par_symmetric _ _ _ _ Par_A_G_F_B) as Par_F_B_A_G.
	
	assert (Square_A_B_F_G_2 := Square_A_B_F_G).
	destruct Square_A_B_F_G_2 as (
		_ & Cong_A_B_B_F & _ & RightTriangle_G_A_B & RightTriangle_A_B_F & _ & _
	).
		
	pose proof (by_prop_Cong_flip _ _ _ _ Cong_A_B_B_F) as (Cong_B_A_F_B & Cong_B_A_B_F & Cong_A_B_F_B).
	pose proof (by_prop_Cong_symmetric _ _ _ _ Cong_A_B_B_F) as Cong_B_F_A_B.
	pose proof (by_prop_Cong_flip _ _ _ _ Cong_B_F_A_B) as (Cong_F_B_B_A & Cong_F_B_A_B & Cong_B_F_B_A).

	pose proof (lemma_righttogether _ _ _ _ RightTriangle_G_A_B RightTriangle_B_A_C OppositeSide_G_B_A_C) as (_ & BetS_G_A_C).
	...
	pose proof (by_prop_Par_collinear _ _ _ _ _ Par_F_B_A_G Col_A_G_C neq_C_G) as Par_F_B_C_G.
	pose proof (by_prop_Par_flip _ _ _ _ Par_F_B_C_G) as (_ & Par_F_B_G_C & _).

	assert (Par_F_B_G_C_2 := Par_F_B_G_C).
	destruct Par_F_B_G_C_2 as (_ & _ & _ & _ & _ & neq_F_B & _ & _ & _ & _ & _ & _ & _ & n_Meet_F_B_G_C & _ & _).

	pose proof (by_prop_neq_symmetric _ _ neq_F_B) as neq_B_F.

	pose proof (by_def_OnRay_from_neq_A_B B F neq_B_F) as OnRay_B_F_F.

	pose proof (by_prop_RightTriangle_symmetric _ _ _ RightTriangle_A_B_F) as RightTriangle_F_B_A.

	pose proof (by_prop_OppositeSide_flip _ _ _ _ OppositeSide_G_B_A_C) as OppositeSide_G_A_B_C.
	pose proof (lemma_planeseparation _ _ _ _ _ SameSide_F_G_A_B OppositeSide_G_A_B_C) as OppositeSide_F_A_B_C.
	...
	destruct Parallelogram_B_C_E_D_2 as (_ & Par_B_D_C_E).
	pose proof (by_prop_Par_symmetric _ _ _ _ Par_B_D_C_E) as Par_C_E_B_D.
	pose proof (by_prop_Par_flip _ _ _ _ Par_C_E_B_D) as (_ & Par_C_E_D_B & _).
	pose proof (by_prop_Par_NC _ _ _ _ Par_C_E_D_B) as (_ & nCol_C_D_B & _ & _).
	...
	pose proof (by_prop_CongA_reflexive _ _ _ nCol_D_B_C) as CongA_D_B_C_D_B_C.
	...
	destruct Square_B_C_E_D_2 as (
		_ & _ & Cong_B_C_D_B & RightTriangle_D_B_C & _ & _ &_ 
	).

	pose proof (by_prop_Cong_symmetric _ _ _ _ Cong_B_C_D_B) as Cong_D_B_B_C.
	pose proof (by_prop_Cong_flip _ _ _ _ Cong_D_B_B_C) as (_ & Cong_B_D_B_C & _).

	pose proof (lemma_Euclid4 _ _ _ _ _ _ RightTriangle_F_B_A RightTriangle_D_B_C) as CongA_F_B_A_D_B_C.

	pose proof (proposition_47A _ _ _ _ _ Triangle_A_B_C RightTriangle_B_A_C Square_B_C_E_D OppositeSide_D_C_B_A) as (
		M & L & Parallelogram_B_M_L_D & BetS_B_M_C & Parallelogram_M_C_E_L & BetS_D_L_E & BetS_L_M_A & RightTriangle_D_L_A
	).

	pose proof (proposition_34 _ _ _ _ Parallelogram_B_M_L_D) as (_ & _ & _ & _ & CongTriangles_M_B_D_D_L_M).
	pose proof (axiom_congruentequal _ _ _ _ _ _ CongTriangles_M_B_D_D_L_M) as EqAreaTri_M_B_D_D_L_M.

	assert (Parallelogram_B_M_L_D_2 := Parallelogram_B_M_L_D).
	destruct Parallelogram_B_M_L_D_2 as (Par_B_M_L_D & Par_B_D_M_L).
	pose proof (by_prop_Par_flip _ _ _ _ Par_B_M_L_D) as (_ & _ & Par_M_B_D_L).
	pose proof (by_prop_Par_symmetric _ _ _ _ Par_B_D_M_L) as Par_M_L_B_D.
	pose proof (by_def_Parallelogram _ _ _ _ Par_M_B_D_L Par_M_L_B_D) as Parallelogram_M_B_D_L.
	...
	pose proof (by_prop_Par_collinear _ _ _ _ _ Par_B_D_M_L Col_M_L_A neq_A_L) as Par_B_D_A_L.
	pose proof (by_prop_Par_flip _ _ _ _ Par_B_D_A_L) as (_ & _ & Par_D_B_L_A).
	assert (Par_D_B_L_A_2 := Par_D_B_L_A).
	destruct Par_D_B_L_A_2 as (_ & _ & _ & _ & _ & _ & _ & _ & _ & _ & _ & _ & _ & n_Meet_D_B_L_A & _ & _).

	destruct OppositeSide_F_A_B_C as (a & BetS_F_a_C & Col_A_B_a & nCol_A_B_F).
	...
	pose proof (by_prop_CongA_reflexive _ _ _ nCol_F_B_A) as CongA_F_B_A_F_B_A.
	...
	pose proof (by_def_OnRay_from_BetS_A_B_E B a A BetS_B_a_A neq_B_a) as OnRay_B_a_A.
	pose proof (by_prop_OnRay_ABC_ACB _ _ _ OnRay_B_a_A) as OnRay_B_A_a.

	pose proof (by_prop_CongA_helper _ _ _ _ _ _ _ _ CongA_F_B_A_F_B_A OnRay_B_F_F OnRay_B_A_a) as CongA_F_B_A_F_B_a.
	pose proof (by_prop_CongA_helper _ _ _ _ _ _ _ _ CongA_A_B_C_A_B_C OnRay_B_A_a OnRay_B_C_C) as CongA_A_B_C_a_B_C.
	pose proof (by_def_AngleSum _ _ _ _ _ _ _ _ _ _ CongA_F_B_A_F_B_a CongA_A_B_C_a_B_C BetS_F_a_C) as AngleSum_F_B_A_A_B_C_F_B_C.

	assert (OppositeSide_D_C_B_A_2 := OppositeSide_D_C_B_A).
	destruct OppositeSide_D_C_B_A_2 as (c & BetS_D_c_A & Col_C_B_c &_ ).

	pose proof (by_def_Col_from_BetS_A_B_C D c A BetS_D_c_A) as Col_D_c_A.
	...
	pose proof (by_def_OnRay_from_BetS_A_B_E B c C BetS_B_c_C neq_B_c) as OnRay_B_c_C.
	pose proof (by_prop_OnRay_ABC_ACB _ _ _ OnRay_B_c_C) as OnRay_B_C_c.

	pose proof (by_prop_CongA_helper _ _ _ _ _ _ _ _ CongA_C_B_A_C_B_A OnRay_B_C_c OnRay_B_A_A) as CongA_C_B_A_c_B_A.
	pose proof (by_prop_CongA_helper _ _ _ _ _ _ _ _ CongA_D_B_C_D_B_C OnRay_B_D_D OnRay_B_C_c) as CongA_D_B_C_D_B_c.
	pose proof (by_def_AngleSum _ _ _ _ _ _ _ _ _ _ CongA_D_B_C_D_B_c CongA_C_B_A_c_B_A BetS_D_c_A) as AngleSum_D_B_C_C_B_A_D_B_A.

	pose proof (
		by_prop_AngleSum_respects_congruence
		_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _
		AngleSum_F_B_A_A_B_C_F_B_C CongA_F_B_A_D_B_C CongA_A_B_C_C_B_A AngleSum_D_B_C_C_B_A_D_B_A
	) as CongA_F_B_C_D_B_A.
	pose proof (by_prop_CongA_symmetric _ _ _ _ _ _ CongA_F_B_C_D_B_A) as CongA_D_B_A_F_B_C.

	pose proof (by_prop_CongA_NC _ _ _ _ _ _ CongA_D_B_A_F_B_C) as nCol_F_B_C.
	pose proof (by_prop_CongA_ABCequalsCBA _ _ _ nCol_F_B_C) as CongA_F_B_C_C_B_F.

	pose proof (by_prop_CongA_transitive _ _ _ _ _ _ _ _ _ CongA_D_B_A_F_B_C CongA_F_B_C_C_B_F) as CongA_D_B_A_C_B_F.

	pose proof (proposition_04 _ _ _ _ _ _ Cong_B_D_B_C Cong_B_A_B_F CongA_D_B_A_C_B_F) as (
		Cong_D_A_C_F & _ & _
	).

	pose proof (by_prop_Cong_flip _ _ _ _ Cong_D_A_C_F) as (Cong_A_D_F_C & _ & _).
	pose proof (by_def_CongTriangles _ _ _ _ _ _ Cong_A_B_F_B Cong_B_D_B_C Cong_A_D_F_C) as CongTriangles_A_B_D_F_B_C.
	pose proof (axiom_congruentequal _ _ _ _ _ _ CongTriangles_A_B_D_F_B_C) as EqAreaTri_A_B_D_F_B_C.
