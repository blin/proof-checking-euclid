...
def prolog_rule_for_lemma(req: Lemma) -> str:
    ...
    match req.conclusion:
        ...
        case PropExists(points=_, p=PropConjunction()):
            conclusions = collect_conjunction_nodes(req.conclusion.p)
    ...
    match req.conclusion:
        ...
        case PropExists(points=_, p=PropSimple()):
            ...
            r.write(".\n")
        case PropConjunction() | PropExists(points=_, p=PropConjunction()):
            r.write(
                ",\n".join(
                    [prolog_lemma_rule(p, f"C{i+1}", p_to_i) for i, p in enumerate(conclusions)]
                )
            )
            r.write(".\n")
        case _:
...
# TODO: merge prove_simple, prove_forward_using, prove_conclude_exists and prove_conclude_conjunction
def prove_simple(
    p: PropSimple, by: Lemma, prolog_lemmas_path: str, prolog_context_path: str
) -> str:
    prolog = Prolog()
...
def prove_conclude_exists(
    p: PropExists, by: Lemma, prolog_lemmas_path: str, prolog_context_path: str
) -> Tuple[str, int]:
    prolog = Prolog()
    prolog.consult(prolog_lemmas_path)
    prolog.consult(prolog_context_path)
    ...
    given = by.given
    assert isinstance(by.conclusion.p, PropConjunction)
    conclusions = collect_conjunction_nodes(p.p)
    gids = [f"G{i+1}" for i in range(len(given))]

    cids = [prolog_prop(c) for c in conclusions]
    pq_given = ",".join(gids)
    pg_conclusion = ",".join(cids)
    pq_args = f"{pq_given},{pg_conclusion}"
    q = f"{by.name}({pq_args})"
...
def prove_conclude_conjunction(
    p: PropConjunction, by: Lemma, prolog_lemmas_path: str, prolog_context_path: str
) -> Tuple[str, int]:
    prolog = Prolog()
    prolog.consult(prolog_lemmas_path)
    prolog.consult(prolog_context_path)
    ...
    given = by.given
    assert isinstance(by.conclusion, PropConjunction)
    conclusions = collect_conjunction_nodes(p)
    gids = [f"G{i+1}" for i in range(len(given))]
